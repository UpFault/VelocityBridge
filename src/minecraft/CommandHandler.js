...
class CommandHandler {
  ...
  handle(player, message) {
    if (message.startsWith(this.prefix)) {
      ...
      const now = Date.now();
      const globalCooldown = 5 * 1000; // 5 seconds in milliseconds

      if (this.lastGlobalCooldown && now - this.lastGlobalCooldown < globalCooldown) {
        return bot.chat(`/gc There is a 5s global cooldown try again later.`);
      }

      this.lastGlobalCooldown = now;
      ...
      (async () => {
        try {
          ...
          if (response?.data?.msg === undefined) {
            return bot.chat(`/gc An error occured while running the command`);
          }

          bot.chat(`/gc ${response.data.msg}`);
        } catch (e) {
          bot.chat(`/gc ${e.cause ?? e.message ?? "Unknown error"}`);
        }
      })();
    }
  }
